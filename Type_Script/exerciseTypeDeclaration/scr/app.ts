...
function saveSpaceShip(name: string, pilot: string, crewLimit: number, crew?: string[],) {
    const ship = {
        name: name,
        pilot: pilot,
        crewLimit: crewLimit,
...
function addCrew(SpaceShip: any, memberName: string) {
...
function addMission(SpaceShip: any,) {
    ...
    }else {alert(`minimum number of ${SpaceShip.name} crew not reached.`)}
}
...
function shipsListAlert() {
    if (shipsList.length > 0) {
        ...
        alert(strList)
    } else {alert('no registered ships.')}
}
